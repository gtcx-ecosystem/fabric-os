...
**Methodology:** [`gtcx-agentic/audit/prompts/audit/forensic-full-audit-prompt.md`](https://github.com/gtcx-ecosystem/gtcx-agentic/blob/main/audit/prompts/audit/forensic-full-audit-prompt.md), six phases, repo overlay applied per [`01-docs/05-audit/repo-overlay.md`](./repo-overlay.md).  
...
- **[P1] `03-platform/tools/audit-flush/Dockerfile` does not bundle `@gtcx/audit-signer`.** The runtime `import '@gtcx/audit-signer'` will fail without the package present in `node_modules`. Unblocked by the npm publish on 2026-05-22 — the Dockerfile's `npm install` can now pull it like any other dependency.
- **[P1] Adaptive policy state is per-pod, not global.** Acceptable through pilot scale; needs Redis-backed shared state at >10 pods.
- **[P2] `03-platform/tools/audit-flush/` lacks an integration test against a real NATS broker.** Add a docker-compose-backed integration test under `validate.sh --full` when production deploy is imminent.
...
| Audit-flush IRSA scope          | **Closed**           | `04-deploy/terraform/modules/audit-flush-irsa/main.tf` write-only PutObject + KMS:Encrypt; no GetObject, no DeleteObject                                                                                                                       |
| Dev creds in scripts            | **Closed**           | `04-deploy/03-platform/scripts/dr-test.sh` and `.github/workflows/ci.yml` fail-fast via `:?` guards                                                                                                                                            |
...
1. **Leadership sends the pen-test RFP** — artifact at `01-docs/05-audit/pen-test-rfp-2026.md`.
2. **Leadership sends the SOC 2 outreach** — artifact at `01-docs/05-audit/soc2-engagement-2026.md`.
3. **Audit-flush container image built + pushed** — `03-platform/tools/audit-flush/Dockerfile` exists; needs `docker build && push` once.
...
| File Structure    | 9.5   | 11 workspace packages match `03-platform/tools/*/` layout; no orphan files      |
...
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------- | --------- | -------------------------------------------- |
| A   | Onboard a synthetic second tenant via `01-docs/04-ops/runbooks/tenant-onboarding.md`; validate the 2-hour SLA empirically                      | 2 hours   | Validates the tenant boundary work           |
...
- Repo overlay: [`01-docs/05-audit/repo-overlay.md`](./repo-overlay.md)
- Master audit: [`01-docs/05-audit/master-audit-2026-05-17.md`](./master-audit-2026-05-17.md)
- SIGNAL scorecard: [`01-docs/05-audit/signal-scorecard.json`](./signal-scorecard.json)
- Score evidence ledger: [`01-docs/05-audit/score-evidence-ledger.json`](./score-evidence-ledger.json)
- Coverage gate rationale: [`01-docs/05-audit/coverage-gate-rationale.md`](./coverage-gate-rationale.md)
