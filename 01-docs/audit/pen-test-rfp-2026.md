...
We have run an internal red-team against the gateway using the injection-suite at `03-platform/tools/eval-pipeline/injection-suite.mjs`. The full pre-engagement evidence ledger is in [`01-docs/05-audit/score-evidence-ledger.json`](./score-evidence-ledger.json). We do not consider internal red-teaming a substitute for external pen-test.
...
**Status:** Awaiting leadership send. The pen-test engagement is the single biggest external blocker for stage gate S2→S3 (per `01-docs/05-audit/master-audit-2026-05-17.md:135-141`).
