...
**Framework Reference:** `01-docs/05-audit/audit-grade-framework.md` §2.2  
...
cat 01-docs/10-compliance/soc2-gap-analysis.md 2>/dev/null | head -30 || echo "No SOC 2 gap analysis found"
# Check for auditor engagement
grep -ri "soc.*2\|iso.*27001" 01-docs/05-audit/ | head -10
...
cat 01-docs/05-audit/pen-test-scope-2026.md | head -20
# Check vendor shortlist
cat 01-docs/05-audit/pen-test-vendor-shortlist.md | head -20
# Check for historical pen-test reports
find 01-docs/05-audit/ -name "*pen*test*report*" -o -name "*pentest*" | head -5
```
...
find 01-docs/05-audit/ -name "*uptime*" -o -name "*availability*" | head -5
```
...
cat 01-docs/09-security/vulnerability-disclosure-policy.md 2>/dev/null | head -20 || echo "No VDP found"
```
...
find 01-docs/ -name "*bcp*" -o -name "*business*continuity*" -o -name "*disaster*recovery*" | head -10
# Check RTO/RPO definitions
grep -ri "rto\|rpo\|recovery.*time\|recovery.*point" 01-docs/devops/ | head -10
# Check DR test schedule
find 01-docs/ -name "*dr*test*" -o -name "*drill*" | head -5
```
...
find 01-docs/ -name "*vendor*" -o -name "*questionnaire*" -o -name "*vsa*" | head -10
# Check for security controls matrix
find 01-docs/10-compliance/ -name "*controls*" -o -name "*matrix*" | head -5
```
...
grep -ri "retention\|deletion\|destruction" 01-docs/10-compliance/ 01-docs/09-security/ | head -10
# Check for certificate of destruction template
find 01-docs/ -name "*destruction*" -o -name "*deletion*cert*" | head -5
```
...
Produce a JSON file at `01-docs/05-audit/prompts/evidence/enterprise-grade-evidence.json`:
...
- `01-docs/05-audit/audit-grade-framework.md` §2.2
- `01-docs/10-compliance/soc2-gap-analysis.md`
- `01-docs/05-audit/pen-test-scope-2026.md`
- `01-docs/05-audit/pen-test-vendor-shortlist.md`
