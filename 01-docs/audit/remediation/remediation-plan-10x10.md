...
- **Fix:** Add a clear comment block above the `secretGenerator` section:
  ...
  # See: 01-docs/secrets-management.md
  # ============================================================
  ```
...
- **Audit finding:** Only one deployment script exists (`03-platform/scripts/deploy-intelligence.sh`). No comprehensive deployment runbook in `01-docs/`.
- **Evidence:** `01-docs/` — no deployment runbook found
...
- **Verification:** `test -f 01-docs/deployment-runbook.md` and file contains sections for prerequisites, setup, deployment, secrets, verification, rollback
...
- [ ] `01-docs/deployment-runbook.md` exists with prerequisites, setup, deployment, secrets, verification, rollback, network CIDR sections
...
grep -c 'Rollback' 01-docs/deployment-runbook.md  # >= 1
