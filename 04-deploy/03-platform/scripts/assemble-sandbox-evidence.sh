...
if [ -f "${SANDBOX_DIR}/architecture-overview.md" ]; then
  ...
  log_warn "01 — Architecture overview not found at 01-docs/08-gtm/sandbox-application/architecture-overview.md"
fi
...
if [ -f "${REGULATORY_DIR}/incident-response-plan-v1.md" ]; then
  ...
  log_warn "07 — Incident response plan not found at 01-docs/08-gtm/regulatory/incident-response-plan-v1.md"
fi
