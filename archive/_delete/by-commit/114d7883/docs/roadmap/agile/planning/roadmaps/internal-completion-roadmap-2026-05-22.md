...
- [ ] `03-platform/tools/audit-flush/test/docker-compose.test.yml` starts a single-node `nats:2-alpine` with JetStream enabled.
...
- [ ] `03-platform/tools/audit-flush/Dockerfile` installs `@gtcx/audit-signer@^0.1.0` in the `deps` stage alongside `nats` and `@aws-sdk/client-s3`.
...
1. `docker build -t test/audit-flush 03-platform/tools/audit-flush` succeeds.
...
| Audit-flush image builds  | `docker build 03-platform/tools/audit-flush` | Success; container starts             | Platform Eng | ☐      |
...
- [ ] Each ADR follows `01-docs/architecture/decisions/adr-template.md` (status, context, decision, consequences, alternatives).
- [ ] All four linked in `01-docs/architecture/decisions/README.md`.
...
| ADR registry updated     | Open `01-docs/architecture/decisions/README.md` | Lists ADRs 014–017                        | Doc Lead     | ☐      |
...
- [ ] `01-docs/gitbook/blog/audit-signer-launch-2026-05.md` (new) — ~1500 words.
...
- [ ] `01-docs/gitbook/docs-site/` (new directory) — markdown source for the site.
...
- [ ] `01-docs/gitbook/terraform-registry-submission-2026.md` (new) — checklist of Registry requirements with each verified.
...
| Docs site links     | `docs-link-checker 01-docs/external`        | All resolve         | Doc Lead     | ☐      |
...
- [ ] `01-docs/10-compliance/dpia-2026-05.md` (new) — schema-compliant frontmatter.
...
- [ ] `01-docs/10-compliance/soc2-evidence-inventory-2026-05.md` (new) — frontmatter compliant.
...
- [ ] Includes the evidence already mapped in `01-docs/05-audit/soc2-engagement-2026.md` plus deeper-cut pointers (e.g., per-test-file for code-quality controls).
