...
ir_roadmap: 01-docs/05-audit/ir-10-10-roadmap.md
sources:
  - 01-docs/05-audit/ir-10-10-roadmap.md
  - 01-docs/05-audit/post-roadmap-session-2026-05-30.md
  - 01-docs/05-audit/master-audit-2026-05-30.md
  - 01-docs/05-audit/master-audit-2026-06-02.md
  - 01-docs/05-audit/bank-grade-audit-2026-06-07.md
  - 01-docs/05-audit/engineering-audit-2026-06-07.md
  ...
  - 01-docs/roadmap/roadmap-2026-07-13.md
  - 01-docs/08-gtm/overview.md
  - 01-docs/08-gtm/plans/global-south-10x-plan.md
  - 01-docs/08-gtm/plans/moat-execution-plan.md
  - 01-docs/08-gtm/plans/bank-grade-10x-remediation-plan.md
  - 01-docs/08-gtm/regulatory/soc2-readiness-checklist.md
  - 01-docs/08-gtm/regulatory/incident-response-plan-v1.md
  - 01-docs/08-gtm/regulatory/pentest-scope-rfp.md
  - 01-docs/08-gtm/sandbox-application/README.md
  - 01-docs/10-compliance/verification-sla-draft.md
...
S4-04/05/06/07/08 **done** (2026-06-05). Hub **#17 closed** 2026-06-05. `backlogClear: true` — witness + EXT-INF human gates parallel.
...
**Score baseline (2026-06-07, rubric v2):** **IR 7.9** + **bank-grade 8.3** + **XC 9.0**. `validate-all` **55/55** @ 2026-06-05. **repoHygiene 9.8** ([`repo-hygiene-2026-06-05.md`](./repo-hygiene-2026-06-05.md)). Witness mode (`backlogClear: true`).

**Reconcile delta (2026-06-05 execute-roadmap):** **S4-08 done** — 33 docs-standard violations cleared (agent README path fixes, KIMI/CODEX stubs, cross-repo GitHub URLs, topology rename, missing indexes). **S4-07 unblocked**. `validate-all` 55/55. Human blockers unchanged: EXT-INF-002/013/014/015/016.

**Reconcile delta (2026-06-05):** **Hub #17 closed** — prod `compliance.gtcx.trade` + `terminal.gtcx.trade`; baseline-os locker `7d98352b2`; infra `dee331c`/`e72c728`. **repo-hygiene 8.8** (+0.2). **S4-07** shellcheck fixed; blocked on **docs-standard 27 violations**. Next automatable: **S4-08**. Human blockers unchanged: EXT-INF-002/013/014/015/016.

**Reconcile delta (2026-06-08):** **S4-06 done** — README sweep (12 dirs). Superseded by repo-hygiene 2026-06-05: 5 README gaps remain (P1).

**Reconcile delta (2026-06-07):** Lane audits filed — engineering **7.9**, bank-grade **8.3**. **S4-04/05 done** (gate signoff restore).
...
```bash
node --test 03-platform/tools/replay-protection/tests/**/*.test.mjs
...
```bash
node --test 03-platform/tools/compliance-gateway/tests/audit-bundles/handler.test.mjs
node --test 03-platform/tools/compliance-gateway/tests/**/*.test.mjs
```
...
**Files:** `01-docs/roadmap/roadmap-2026-07-13.md`, `01-docs/roadmap/README.md`
...
**Files:** `01-docs/05-audit/agile/pilot-success-criteria.md:38`, `01-docs/05-audit/external-dependencies-register-2026-05-31.md` (new)
...
test -f 01-docs/05-audit/external-dependencies-register-2026-05-31.md
grep -q "ZWCMP" 01-docs/05-audit/external-dependencies-register-2026-05-31.md
```
...
**Files:** `03-platform/tools/compliance-data/03-platform/scripts/verify-catalog.mjs:60-64`
...
```bash
node --test 03-platform/tools/compliance-data/03-platform/scripts/*.test.mjs
EXPECTED_PUBLIC_KEY=$VALID_KEY node 03-platform/tools/compliance-data/03-platform/scripts/verify-catalog.mjs
EXPECTED_PUBLIC_KEY=$WRONG_KEY node 03-platform/tools/compliance-data/03-platform/scripts/verify-catalog.mjs && exit 1 || echo OK
```
...
**Blockers:** Need leadership confirmation of which key is the canonical trust anchor (current Ed25519 from `03-platform/tools/compliance-data/.keys/` or a new HSM-issued one).
...
| IR-2.3 | CodeQL / Trivy SARIF upload graceful when Code Security disabled      | **done** — `ci.yml` `upload: false` on CodeQL analyze; `continue-on-error: true` on both SARIF upload steps                                       |
...
> `01-docs/05-audit/post-roadmap-session-2026-05-30.md` §Sprint 2 plan.
...
| S3-02 | First quarterly DR fire-drill against staging RDS — evidence committed                   | **done** (`2026-06-04`) — live PITR staging operational; RTO ~20m, RPO 0; side instance deleted; evidence in `01-docs/05-audit/evidence/rds-restore/` |
| S3-03 | Soak-test baseline as PR gate (>25% regression fails build)                              | **done** — `01-docs/05-audit/soak-baseline.json` + `soak-baseline-check.mjs` in validate-all                                                          |
