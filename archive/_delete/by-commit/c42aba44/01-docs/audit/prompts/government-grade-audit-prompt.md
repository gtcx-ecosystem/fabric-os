...
**Framework Reference:** `01-docs/05-audit/audit-grade-framework.md` §2.5  
...
cat 01-docs/05-audit/prompts/evidence/bank-grade-evidence.json 2>/dev/null | jq '.final_score' || echo "Run B-GAP first"
# Verify Bank-Grade ≥ 9.0
cat 01-docs/05-audit/prompts/evidence/bank-grade-evidence.json 2>/dev/null | jq '.final_score >= 9.0' || echo "false"
```
...
cat 01-docs/10-compliance/sita-framework.md 2>/dev/null | head -40 || echo "No SITA framework doc"
# Check for 3PAO engagement
find 01-docs/10-compliance/ -name "*3pao*" -o -name "*assessment*org*" | head -5
# Check for System Security Plan (SSP)
cat 01-docs/10-compliance/ssp-fedramp.md 2>/dev/null | head -40 || echo "No SSP"
# Check for control implementation evidence
find 01-docs/10-compliance/ -name "*control*" | wc -l
```
...
cat 01-docs/10-compliance/fips140-2-assessment.md 2>/dev/null | head -40 || echo "No FIPS assessment"
...
cat 01-docs/hr/personnel-security-policy.md 2>/dev/null | head -40 || echo "No personnel security policy"
# Check for clearance requirements in job descriptions
grep -ri "clearance\|citizenship\|security.check" 01-docs/hr/ | head -10
# Check for background check process
grep -ri "background.check\|vetting\|screening" 01-docs/hr/ | head -10
```
...
grep -ri "multi.region\|failover\|dr\|disaster" 01-docs/architecture/ | head -10
...
cat 01-docs/governance/king-iv-compliance.md 2>/dev/null | head -40 || echo "No King IV doc"
# Check for board charter
cat 01-docs/governance/board-charter.md 2>/dev/null | head -30 || echo "No board charter"
# Check for ethics policy
cat 01-docs/governance/ethics-policy.md 2>/dev/null | head -30 || echo "No ethics policy"
# Check for audit committee
cat 01-docs/governance/audit-committee-terms.md 2>/dev/null | head -30 || echo "No audit committee terms"
```
...
cat 01-docs/10-compliance/popia-registration.md 2>/dev/null | head -30 || echo "No POPIA registration doc"
# Check for ROPA (Record of Processing Activities)
cat 01-docs/10-compliance/popia-ropa.md 2>/dev/null | head -40 || echo "No ROPA"
# Check for breach notification process
cat 01-docs/10-compliance/popia-breach-notification.md 2>/dev/null | head -30 || echo "No breach notification process"
# Check for FICA compliance
cat 01-docs/10-compliance/fica-compliance.md 2>/dev/null | head -40 || echo "No FICA compliance doc"
# Check for PAIA manual
cat 01-docs/10-compliance/paia-manual.md 2>/dev/null | head -40 || echo "No PAIA manual"
```
...
Produce a JSON file at `01-docs/05-audit/prompts/evidence/government-grade-evidence.json`:
...
- `01-docs/05-audit/audit-grade-framework.md` §2.5
- `01-docs/05-audit/prompts/bank-grade-audit-prompt.md`
- `01-docs/governance/king-iv-compliance.md`
- `01-docs/10-compliance/popia-registration.md`
- `01-docs/10-compliance/fica-compliance.md`
