# Global compliance, security & trust — fabric-os (2026-06-11)

**Score:** 100/100 (raw 100) · **Status:** rubric-driven

## Metrics (0–100 rubric categories)

| Dimension               | Score | Weight | Confidence | Source       |
| ----------------------- | ----: | -----: | ---------- | ------------ |
| security                |   100 |      1 | C          | audit-output |
| enterprise-readiness    |   100 |      1 | C          | audit-output |
| bank-grade-auditability |   100 |      1 | C          | audit-output |

## Gates (pass/fail ceilings)

| Gate                          | Result | Severity | Cap if fail |
| ----------------------------- | ------ | -------- | ----------- |
| security-scan-clean-or-waived | PASS   | P0       | 59          |
| secrets-no-leak               | PASS   | P0       | 50          |
| trust-evidence-indexed        | PASS   | P1       | 75          |

## Caps fired

- none

## Gaps

- none
