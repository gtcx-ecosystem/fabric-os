#!/usr/bin/env bash
# Point commops-staging.gtcx.trade at staging ALB (CommOps bounce webhooks).
...
#   ./attach-commops-staging-domain.sh
set -euo pipefail
ROOT="$(cd "$(dirname "$0")" && pwd)"
export CLOUDFLARE_COMPLIANCE_HOST="commops-staging"
