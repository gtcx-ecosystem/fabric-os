#!/usr/bin/env bash
# =============================================================================
# GTCX Staging Environment Bootstrap
# =============================================================================
# One-shot script to prepare the GTCX staging environment.
...
#   5. Creates GitHub OIDC provider for CI/CD
#   6. Initializes Terraform for staging environment
#
# Prerequisites:
...
#
# Usage:
#   ./03-platform/scripts/bootstrap-staging.sh
#   ./03-platform/scripts/bootstrap-staging.sh --skip-region    # If af-south-1 already enabled
#   ./03-platform/scripts/bootstrap-staging.sh --skip-oidc      # Skip GitHub OIDC setup
#
# Estimated time: 5-10 minutes (region enablement can take 1-2 minutes)
# Estimated monthly cost after apply: ~$350-500
# =============================================================================
...
echo ""
echo "=============================================="
echo "  GTCX Staging Environment Bootstrap"
echo "  Target: af-south-1 (Cape Town)"
echo "=============================================="
echo ""
...
if aws s3api head-bucket --bucket "${STATE_BUCKET}" --region "${TARGET_REGION}" 2>/dev/null; then
    ...
    aws s3api put-bucket-tagging \
        --bucket "${STATE_BUCKET}" \
        --region "${TARGET_REGION}" \
        --tagging 'TagSet=[{Key=Project,Value=gtcx},{Key=Environment,Value=staging},{Key=ManagedBy,Value=bootstrap-script}]'
...
if aws dynamodb describe-table --table-name "${LOCK_TABLE}" --region "${TARGET_REGION}" &>/dev/null; then
    ...
    aws dynamodb create-table \
        ...
        --region "${TARGET_REGION}" \
        --tags Key=Project,Value=gtcx Key=Environment,Value=staging Key=ManagedBy,Value=bootstrap-script
...
log_step "6/6 — Initializing Terraform for staging..."
...
echo "     aws eks update-kubeconfig --name gtcx-staging --region ${TARGET_REGION}"
echo ""
echo "  4. Set GitHub repository variables:"
echo "     AWS_ROLE_ARN       → <deploy_role_arn output from terraform>"
echo "     ECR_REGISTRY       → ${AWS_ACCOUNT_ID}.dkr.ecr.${TARGET_REGION}.amazonaws.com"
echo "     AWS_PROD_ROLE_ARN  → <production deploy role (separate account)>"
echo ""
echo "  5. Deploy gtcx-intelligence services:"
echo "     cd /path/to/gtcx-intelligence"
echo "     # Images are built and pushed by CI on merge to main"
echo ""
echo "  Estimated monthly cost: ~\$350-500 (staging config)"
