...
Modes:
  quick  Run policy checks, shell syntax checks, shellcheck, script smoke tests, 01-docs/ledger gates, and incident-drill validation
...
run_load_tests() {
    ...
    if ! command -v k6 >/dev/null 2>&1; then
        ...
        log_error "k6 not installed — install per 01-docs/04-ops/runbooks/load-testing.md or set GTCX_SKIP_LOAD_TESTS=1"
        return 1
    fi
