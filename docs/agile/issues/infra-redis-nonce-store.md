...
- **Manifest:** `infra/kubernetes/base/services/redis.yaml`
...
`tools/compliance-gateway/src/server.mjs` now initializes the nonce store via:
...
- [x] Redis manifest created (`infra/kubernetes/base/services/redis.yaml`)
...
- [ ] **Deploy Redis:** `kubectl apply -k infra/kubernetes/overlays/staging/`
