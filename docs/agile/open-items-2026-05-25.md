...
- **Our follow-up:** Uncomment `TRADEPASS_BASE_URL` and `AUDIT_BUNDLES_AUDIENCE` in `infra/kubernetes/overlays/production/patches/compliance-gateway-env.yaml`. This is a config-only change — no redeploy needed.
...
- **How:** Go to Actions → "Deploy to Staging" → "Run workflow", or push any change to `infra/kubernetes/**` on `main`.
- **What to verify:**
...
- **What:** `tools/docs-site` fails build on Astro 6 due to legacy content collections (`src/content/config.ts`).
