...
| --------- | ----------------------------------------------- | ----------------- | --------- |
| S1-02     | TypeORM entity/schema drift                     | infra             | `pending` |
| S1-03     | ioredis missing — sovereign image               | infra             | `pending` |
| S1-05     | Terraform IRSA drift capture                    | infra             | `pending` |
...
| S1-05 | **Terraform IRSA drift** — staging IRSA role in state                 | P1       | `pending`  | infra  | `module.irsa_platform` in staging `main.tf`; `terraform plan` shows 0 changes |
...
| S3-06 | **Publish primitives** — `@gtcx/audit-signer`, `terraform-aws-compliance-db` | P2       | `pending`    | devops      | Packages public on npm + Terraform Registry with provenance           |
...
| S3-08 | **Cloudflare Tunnel migration** — `api.gtcx.trade`                           | P2       | `pending`    | infra       | Tunnel routes confirmed; old ALB ingress deprecated                   |
