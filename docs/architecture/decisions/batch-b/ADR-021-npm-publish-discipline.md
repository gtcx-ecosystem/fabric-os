...
Per `01-docs/09-security/threat-model-2026-05.md` T-4 the partial mitigation is GitHub Security Advisories + manual deprecation. That's reactive, not preventive. This ADR codifies the preventive controls.
...
All npm publishes use granular access tokens scoped to the specific package, with "Bypass 2FA" enabled. Account-wide tokens are not used and are not generated. Tokens have a 90-day expiration; rotation is tracked in `01-docs/09-security/credential-rotation-log.md`.
...
- The granular token used for the first publish was rotated immediately after the publish per `01-docs/09-security/credential-rotation-log.md` (the original token was briefly exposed in a chat transcript before rotation).
...
- `01-docs/09-security/threat-model-2026-05.md` — T-4 + SEC-OPEN-002
- `03-platform/tools/audit-signer/package.json` — `prepublishOnly` script enforces the test gate
- `03-platform/tools/audit-signer/CHANGELOG.md` — v0.1.0 publish record
