...
| ------------------------------------------ | ---------------- | ------------------------------------------------------------------------------------------------------ |
| gtcx-core/protocols-crypto                 | 95%              | Cryptographic correctness is existential; a single untested path can compromise the entire trust model |
...
| gtcx-app/03-platform/packages/sync         | 80%              | Offline sync engine correctness is critical to data integrity                                          |
| gtcx-app/packages/events                   | 80%              | Event bus reliability affects all cross-module communication                                           |
| gtcx-app/03-platform/packages/connectivity | 80%              | Network state detection drives degradation behavior                                                    |
...
Property-based testing is mandatory for ALL cryptographic operations in `gtcx-core/protocols-crypto`. The following invariants must be tested with a minimum of 1,000 random inputs per property:
...
Offline tests use a custom `NetworkSimulator` helper that controls `navigator.onLine`, intercepts `fetch`, and simulates latency profiles. See `gtcx-app/03-platform/packages/connectivity/tests/helpers/`.
...
| ------- | ----------------------------------------- | ------------------------------------ | ---------------- |
| Phase 1 | gtcx-core/protocols-crypto property tests | All 9 cryptographic invariants above | Immediate        |
