...
**Reference:** `docs/audit/master-audit-2026-05-10.md`, `docs/remediation/remediation-plan.md`
...
| --------- | --------------------------------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------- |
| **G-001** | No HTTP integration tests for compliance gateway          | **P0**   | `tools/compliance-gateway/tests/server.integration.test.mjs` — 12 tests, all green in CI                    |
...
| **G-003** | No replay-protection production-mode HTTP test            | P2       | `tools/replay-protection/tests/production-fail-closed.test.mjs` — verifies 503 when Redis unavailable       |
| **G-004** | `ai` package import ~8s — load test startup fragile       | P2       | `tools/load-tests/run-load-tests.sh` wait loop increased to 120 iterations (60s max)                        |
| **G-005** | No Kyverno policy deployment evidence in CI               | P2       | `tools/scripts/kyverno-policy-validator.mjs` — structural validation of all 7 policies + kustomization      |
...
| ------------------------------------ | --------------------------------------------------------------- | ----------------------------- |
| Compliance gateway integration tests | `tools/compliance-gateway/tests/server.integration.test.mjs`    | ✅ 12/12 pass                 |
| Replay protection production test    | `tools/replay-protection/tests/production-fail-closed.test.mjs` | ✅ 3/3 pass                   |
| Security control boundaries CI gate  | `.github/workflows/ci.yml`                                      | ✅ Required step              |
| Kyverno policy validator             | `tools/scripts/kyverno-policy-validator.mjs`                    | ✅ 7/7 policies valid         |
| Load test timeout fix                | `tools/load-tests/run-load-tests.sh`                            | ✅ 120 iteration wait         |
| Full validation suite                | `infra/scripts/validate.sh quick`                               | ✅ All pass                   |
| Build evidence                       | `tools/control-plane/generate-release-evidence.mjs`             | ✅ `--build-only` required    |
| Score ledger                         | `docs/audit/score-evidence-ledger.json`                         | ✅ Validator passes           |
| DR test evidence                     | `.github/workflows/dr-test.yml`                                 | ✅ Weekly + on-demand         |
| Incident drill validation            | `tools/scripts/incident-drill-validator.mjs`                    | ✅ Passes against live config |
...
See `docs/audit/remediation-plan-10-10-2026.md` for full detail.
