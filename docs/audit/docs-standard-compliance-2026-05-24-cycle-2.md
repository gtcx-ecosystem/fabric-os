...
**Predecessor:** Cycle 1 self-attestation on the `docs/v2-standard-alignment` branch (PR [#57](https://github.com/gtcx-ecosystem/gtcx-infrastructure/pull/57))
...
Cycle 1 (on the `docs/v2-standard-alignment` branch) scored gtcx-infrastructure at **9.6/10** against Protocol 1 v2.0 + Protocol 13 and documented six follow-up improvements that would push the score toward 10.0.
...
| `20950da` | Axis 1 −0.5 (governance/)            | `docs/governance/README.md` — substantive governance model (3-tier decision model, ADR flow Mermaid, 5-role responsibility table, release-governance matrix, AI model governance for compliance-gateway LLM routing)                                                                                         |
| `1d6abb1` | Axis 1 −0.5 (api/)                   | `docs/api/` folder + `openapi.yaml` (OpenAPI 3.1, 7 endpoints, 12 schemas) — first machine-readable spec for the compliance-gateway HTTP surface                                                                                                                                                             |
...
| `f001ac3` | Axis 3 −0.5                          | YAML frontmatter retrofitted onto 25 folder-index READMEs (zero markdown files in `docs/` now lack YAML frontmatter)                                                                                                                                                                                         |
...
| `86a06d1` | Hygiene cleanup                      | Removed 21 template stubs across `docs/specs/{frontend,design,product,data,testing}/` and `docs/specs/project-specification.md` — all `{Product Name}` placeholders, zero substantive content                                                                                                                |
| `9259317` | Hygiene cleanup                      | Removed 7 template stubs in `docs/engineering/` (`system-architecture-spec`, `content-schema`, `api-specification`, `microservices-architecture`, `database-schema`) + 2 superseded protocols (`agentic-guide`, `architecture-docs-protocol` — both superseded by `gtcx-docs` Protocol 1 v2.0 + Protocol 13) |
...
- `docs/governance/README.md` — substantive (149 lines, Mermaid ADR-flow diagram, 5-role table)
- `docs/api/README.md` + `docs/api/openapi.yaml` — 806-line OpenAPI 3.1 spec covering all 11 documented compliance-gateway endpoints
...
- `grep -L "^---" docs/**/*.md` returns zero files
...
Cycle 1 deducted -1.0 for audience-keyed-vs-legacy-listing redundancy in `docs/README.md`. The runbooks index expansion (commit `c9638a5`) closes part of the discoverability concern at the per-folder level (operations/runbooks/README.md now categorizes 29 runbooks with on-call priority). The README dedup itself remains as the last 0.5-point opportunity.
...
1. **Discoverability +0.5** — collapse the audience-keyed start-here tables and the legacy detailed listing in `docs/README.md` into one canonical form. This is a "decide one shape" call, not engineering work.
...
4. Cycle 1 audit doc (`docs-standard-compliance-2026-05-24.md` on `docs/v2-standard-alignment` branch) as the baseline.
