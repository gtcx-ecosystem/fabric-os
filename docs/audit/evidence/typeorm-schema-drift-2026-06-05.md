...
source_repos:
  - gtcx-infrastructure/infra/docker/init-scripts/postgres/01-schema.sql
  - gtcx-platforms/platforms/*/src/**/*.entity.ts
---

# TypeORM Entity / Schema Drift Report (S1-02)

**Canonical DDL:** `infra/docker/init-scripts/postgres/01-schema.sql` (1,050 lines)  
**TypeORM entities:** `gtcx-platforms/platforms/*/src/**/*.entity.ts` (40+ entities)  
...
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ | ------------------------- |
| 1     | `tradepass_identities`                                                                                                                                                                                                                                                                                                                                        | `platforms/shared/src/operations/tradepass/tradepass-identity.entity.ts` | **XR-302b root cause**    |
| 2     | `audit_records`                                                                                                                                                                                                                                                                                                                                               | `platforms/shared/src/audit/audit-record.entity.ts`                      | Audit logging fails       |
| 3     | `outbox`                                                                                                                                                                                                                                                                                                                                                      | `platforms/shared/src/events/outbox.entity.ts`                           | Event outbox crashes      |
| 4     | `idempotency_keys`                                                                                                                                                                                                                                                                                                                                            | `platforms/shared/src/resilience/idempotency-key.entity.ts`              | Replay protection fails   |
| 5     | `tradepass_credentials`                                                                                                                                                                                                                                                                                                                                       | `platforms/shared/src/operations/tradepass/tradepass.entity.ts`          | Credential issuance fails |
| 6     | `device_keys`                                                                                                                                                                                                                                                                                                                                                 | `platforms/shared/src/crypto/device-key.entity.ts`                       | Device auth fails         |
| 7     | `geotag_proofs`                                                                                                                                                                                                                                                                                                                                               | `platforms/shared/src/operations/geotag/geotag.entity.ts`                | Geotagging fails          |
| 8     | `geotag_sites`                                                                                                                                                                                                                                                                                                                                                | `platforms/shared/src/operations/geotag/geotag-site.entity.ts`           | Site registry fails       |
| 9     | `webhook_subscriptions`                                                                                                                                                                                                                                                                                                                                       | `platforms/shared/src/marketplace/webhook-subscription.entity.ts`        | Webhooks fail             |
| 10    | `compliance_rules`                                                                                                                                                                                                                                                                                                                                            | `platforms/shared/src/compliance/compliance-rule.entity.ts`              | Compliance engine fails   |
| 11–25 | `sync_items`, `sync_counters`, `market_prices`, `roster_assignments`, `sgx_custody_events`, `sgx_revenue_collections`, `crx_permit_documents`, `crx_inspection_evidence`, `pathways_evidence`, `veritas_screening_decisions`, `veritas_inspections`, `veritas_claims`, `veritas_data_stream_subscriptions`, `veritas_confidence_provenance`, `ops_checklists` | Various `platforms/*/src/**/*.entity.ts`                                 | Feature-specific failures |

### 2. Dual Entity Conflict: `tradepass_identities`

Two incompatible entity definitions for the same table:

| Aspect          | Shared Entity (`platforms/shared/src/…`)                                  | TradePass Entity (`platforms/tradepass/src/…`) |
| --------------- | ------------------------------------------------------------------------- | ---------------------------------------------- |
