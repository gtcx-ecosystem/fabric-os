...
| S4-07 | `validate.sh quick` green (shellcheck + tests + docs-standard) | **done** | `bash infra/scripts/validate.sh quick` exit 0 @ 2026-06-05 (S4-08 unblocked docs-standard) |
| S4-08 | Docs-standard drift — agent README stubs + cross-repo links    | **done** | `node tools/scripts/docs-standard-validator.mjs` exit 0; `validate-all` 55/55 @ 2026-06-05 |
...
| S3-02 | First quarterly DR fire-drill against staging RDS — evidence committed                   | **done** (`2026-06-04`) — live PITR staging operational; RTO ~20m, RPO 0; side instance deleted; evidence in `docs/audit/evidence/rds-restore/` |
