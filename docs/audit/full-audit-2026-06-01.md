...
| -------- | ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **P1**   | Ops/CI       | `[P1] Hygiene — main branch CI red on Prettier` — Evidence: Actions run `26743741600`, job `ci`, step `pnpm format:check`; file `docs/audit/distribution-snapshots/2026-06-01.json`. Impact: `main` does not match README "CI-Passing" badge. Fix: `pnpm format --write docs/audit/distribution-snapshots/2026-06-01.json` + commit or add snapshot to `.prettierignore` with documented rationale. |
| **P2**   | Architecture | `[P2] Structural — Bash deploy surface remains authority` — Evidence: `README.md:23-25`, `infra/scripts/deploy.sh:1-59`. Impact: policy logic split between bash and `tools/deployment-guard/`. Fix: continue `gtcx-ctl` migration; gate deploy via `deployment-guard` CLI only.                                                                                                                    |
...
Stages (S0 prototype → S6 defense). Assessment uses `docs/gtm/overview.md`, `pilot-success-criteria.md`, execution roadmap.
...
| File Structure    | **9**     | `infra/` vs `tools/` vs `docs/` clear; ADR and audit supersession pattern                     |
...
- ✗ **CI/CD** — `main` `ci` job failed `format:check` on `docs/audit/distribution-snapshots/2026-06-01.json` (run `26743741600`)
...
| --- | ----------------------------------------------------- | ----------- | --------------------------------------------------- | ------ | --------------------------- |
| 1   | Fix Prettier on distribution snapshot                 | Remediation | `docs/audit/distribution-snapshots/2026-06-01.json` | S      | Unblocks `ci` on every push |
...
| #   | Task                                | Layer       | Files                                                        | Effort    | Why It Matters             |
| --- | ----------------------------------- | ----------- | ------------------------------------------------------------ | --------- | -------------------------- |
| 1   | Assign EXT-INF-013 owner + calendar | Remediation | `docs/audit/pilot-success-criteria.md:27`, external register | S (human) | Unblocks all legal work    |
| 2   | Execute pen-test SOW signature      | Remediation | `docs/gtm/regulatory/pentest-scope-rfp.md`, EXT-INF-002      | M (human) | Bet 1 external validation  |
| 3   | Legal review SLA draft              | Remediation | `docs/compliance/verification-sla-draft.md`, EXT-INF-015     | M (human) | Pilot agreement dependency |
...
| 2   | AI SDK v5→v6 migration branch           | Evolution   | `tools/compliance-gateway/`, `providers.mjs`, tests | L      | Closes #69–73 safely        |
| 3   | Re-run eval-pipeline + compliance tests | Remediation | `tools/eval-pipeline/`, CI                          | M      | Proves NL layer still works |
...
| 2   | Operator live DR against staging RDS  | Remediation | `infra/scripts/dr-test.sh`, evidence dir                        | L (ops) | S3-02 live half               |
| 3   | Staging smoke probe runbook execution | Evolution   | `docs/operations/runbooks/staging-smoke-probe.md`               | M       | Runtime proof                 |
...
| #   | Task                                           | Layer       | Files                                                                       | Effort | Why It Matters        |
| --- | ---------------------------------------------- | ----------- | --------------------------------------------------------------------------- | ------ | --------------------- |
| 1   | Route `deploy.sh` through deployment-guard CLI | Evolution   | `infra/scripts/deploy.sh`, `tools/deployment-guard/src/cli/deploy-gate.mjs` | M      | Single policy brain   |
| 2   | Expand gtcx-ctl validate-environment in CI     | Evolution   | `tools/control-plane/`, `ci.yml`                                            | M      | Pre-flight env checks |
...
| #   | Task                                      | Layer      | Files                                                    | Effort | Why It Matters            |
| --- | ----------------------------------------- | ---------- | -------------------------------------------------------- | ------ | ------------------------- |
| 1   | Pilot KPI dashboard from Prometheus rules | Innovation | `infra/kubernetes/base/monitoring/`, pilot criteria      | M      | Measurable 30-day success |
| 2   | Offline verifier pack for ZWCMP           | Innovation | `tools/compliance-data/scripts/verify-catalog.mjs`, docs | M      | Regulator-facing moat     |
| 3   | USSD path soak test in CI                 | Innovation | `tools/ussd-handler/`, load tests                        | M      | Global South distribution |

### Definition of Done

- Pilot success criteria metrics visible in Grafana or exported report
- Verifier doc committed under `docs/audit/vendor-outreach/zwcmp/`
