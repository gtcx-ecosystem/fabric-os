...
| ----------------------- | ---------------------------------------------------- | ----: | -------: | ---------------------------------------------------- |
| Audit bundles endpoint  | `tools/compliance-gateway/src/audit-bundles/`        |   103 |    98.9% | Ed25519 envelope verify, nonce gate, chain validator |
| Audit query endpoint    | `tools/compliance-gateway/src/audit-query/`          |   160 |    97.7% | Bearer auth, tenant isolation, NDJSON store          |
| Redis nonce store       | `tools/compliance-gateway/src/nonce-store/redis.mjs` |    10 |    52.9% | Cross-service replay protection                      |
...
- **File:** `tools/replay-protection/tests/*.test.mjs`
...
- **File:** `tools/docs-site/astro.config.mjs:24-27`
...
- **File:** `infra/terraform/environments/staging/main.tf:261,294` and `infra/terraform/modules/worm-audit/versions.tf:6-7`
...
- **File:** `tools/compliance-gateway/src/nonce-store/redis.mjs`
...
- **File:** `tools/compliance-gateway/src/server.mjs:560-606`
- **Issue:** `/audit/bundles` and `/audit/query` endpoints lack the per-principal QPS/daily-budget throttling that protects `/v1/query` (`src/budget.mjs:checkBudget`). An authenticated principal could flood these endpoints.
...
- **File:** `tools/docs-site/package.json`
- **Vulnerabilities:**
...
- **File:** `infra/scripts/validate.sh`
...
- **File:** `tools/replay-protection/tests/audit-capture.test.mjs:91-100`
...
- **File:** `infra/kubernetes/base/kustomization.yaml`
