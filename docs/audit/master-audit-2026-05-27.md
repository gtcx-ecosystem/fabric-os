...
**HEAD:** `c27f203`  
**Auditor:** Codex CLI  
**Methodology:** `gtcx-agentic` repository docs, `gtcx-agentic/docs/agent-specs/gtcx-infrastructure.md`, `gtcx-agentic/docs/audit/master-audit-2026-05-26.md`, and canonical audit framework files found at `gtcx-docs/tools/audit/audit-framework/`.

> Note: `gtcx-agentic/CONVENTIONS.md` points to `../gtcx-agentic/tools/audit-framework`, but that directory is absent in this checkout. The canonical framework is present at `gtcx-docs/tools/audit/audit-framework`; this audit used that framework plus the available `gtcx-agentic` repo docs.
...
Executive summary artifact: [`docs/audit/master-audit-summary-2026-05-27.md`](./master-audit-summary-2026-05-27.md).
...
- `docs/audit/repo-overlay.md`
- `docs/audit/prompts/master-audit-prompt.md`
- `gtcx-agentic/CONVENTIONS.md`
- `gtcx-agentic/docs/agent-specs/gtcx-infrastructure.md`
- `gtcx-agentic/docs/audit/master-audit-2026-05-26.md`
- `gtcx-agentic/docs/audit/10-10-roadmap-2026-05-26.md`
- `gtcx-docs/tools/audit/audit-framework/AGENT-START.md`
- `gtcx-docs/tools/audit/audit-framework/commands/master-audit.md`
- `gtcx-docs/tools/audit/audit-framework/SCORING_FRAMEWORK.md`
- `gtcx-docs/tools/audit/audit-framework/prompts/master/forensic-master-prompt.md`

**Dirty tree exception:** the framework asks for a clean worktree, but the repo already contained:

- `M AGENTS.md`
- `?? docs/agile/sprints/current.md`
...
| `pnpm test`                                               | FAIL   | Docs-standard fails on `docs/agile/sprints/current.md` missing `status`, `date`, `owner`.                 |
...
| `terraform fmt -check -recursive infra/terraform/`        | FAIL   | `infra/terraform/environments/staging/main.tf`, `infra/terraform/modules/worm-audit/versions.tf`.         |
| `kubectl kustomize infra/kubernetes/base/`                | PASS   | Builds with `commonLabels` deprecation warning.                                                           |
| `kubectl kustomize infra/kubernetes/overlays/staging/`    | PASS   | Builds with `commonLabels` deprecation warning.                                                           |
| `kubectl kustomize infra/kubernetes/overlays/production/` | FAIL   | Patch targets `audit-flush`, but base resource is removed.                                                |
| `kubectl kustomize infra/kubernetes/overlays/testnet/`    | PASS   | Builds with `commonLabels` deprecation warning.                                                           |
| `kubectl kustomize infra/kubernetes/overlays/pen-test/`   | FAIL   | Namespace ID conflict.                                                                                    |
...
| ------ | ------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| P0-001 | Production Kustomize overlay cannot build. | `infra/kubernetes/overlays/production/kustomization.yaml:60` applies `audit-flush-patch.yaml`, while `infra/kubernetes/base/kustomization.yaml:22-24` comments out `services/audit-flush.yaml`. | Production manifest generation fails; audit-flush deployment cannot be certified. |
| P0-002 | `pnpm build` fails in docs-site.           | `tools/docs-site/astro.config.mjs:24-27` uses array-shaped `social`; Starlight expects an object. Build also prompts for missing `@astrojs/check`.                                              | Release/build gate fails.                                                         |
| P0-003 | Workspace lint fails.                      | `tools/replay-protection/tests/*.test.mjs`: 11 errors, 3 warnings; import order, unused vars, duplicate key.                                                                                    | CI blocks; quality gate not clean.                                                |
| P0-004 | Terraform format gate fails.               | `infra/terraform/environments/staging/main.tf:264`, `:297`; `infra/terraform/modules/worm-audit/versions.tf:6-7`.                                                                               | Full IaC validation fails; overlay cap limits enterprise readiness.               |

### P1 — High

| ID     | Finding                                                                | Evidence                                                                                                                                                          | Impact                                                                      |
| ------ | ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| P1-001 | Hardcoded PagerDuty routing key in repo.                               | `infra/docker/observability/alertmanager.yml:180`; detected by local `gitleaks`.                                                                                  | Potential incident-routing secret exposure and alert-spam risk.             |
| P1-002 | New audit endpoints lack the same QPS/budget gate used by `/v1/query`. | `tools/compliance-gateway/src/server.mjs:220-244` gates `/v1/query`; `/audit/bundles` and `/audit/query` handlers at `:560` and `:589` do not call `checkBudget`. | Authenticated principal can flood audit ingestion/query paths.              |
...
| P1-004 | Pen-test Kustomize overlay cannot build.                               | `infra/kubernetes/overlays/pen-test/kustomization.yaml:20-22` includes base namespace and `namespace.yaml`, causing namespace ID conflict.                        | External pen-test environment cannot be generated as documented.            |
| P1-005 | Docs-standard fails on current checkout.                               | `docs/agile/sprints/current.md` lacks `status`, `date`, and `owner` fields.                                                                                       | `pnpm test` and governance gate fail if the sprint file is committed as-is. |
...
| P1-008 | Overview document is stale.                                            | `docs/overview/README.md` still points to `master-audit-2026-05-17.md` and claims a 5.9 cap from Vault TLS.                                                       | Executive entry point conflicts with current audit evidence.                |

### P2 — Medium

| ID     | Finding                                                           | Evidence                                                                                                                           | Impact                                                                                       |
| ------ | ----------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| P2-001 | Prettier check fails in 8 files.                                  | Includes `docs/audit/master-audit-2026-05-26.md`, distribution snapshots, and `docs/security/partner-security-self-assessment.md`. | Formatting noise blocks CI and review confidence.                                            |
| P2-002 | Local secret scan flagged docs fake key.                          | `docs/agents/workflows/agent-safety-rules.md:225` contained a token-shaped fake API key.                                           | False positives reduce signal; use non-token-shaped examples or allowlist.                   |
...
| P2-004 | Redis nonce-store coverage remains low inside compliance-gateway. | `tools/compliance-gateway/src/nonce-store/redis.mjs`: 52.88% statements, 50% branches.                                             | Redis failover and error-handling behavior are under-tested.                                 |
...
Detailed 10/10 remediation plan: [`docs/audit/10-10-remediation-plan-2026-05-27.md`](./10-10-remediation-plan-2026-05-27.md).
...
1. `pnpm lint`, `pnpm format:check`, `pnpm build`, `pnpm test`, and `terraform fmt -check -recursive infra/terraform/` all pass on a clean checkout.
...
- Phase 2 doc cleanup was skipped because the repo has only `/docs/` and no competing `_sop`, `_cannon`, `wiki`, or `documentation` roots.
...
- `gtcx-agentic/CONVENTIONS.md` points to a missing `gtcx-agentic/tools/audit-framework` path, so the canonical framework available at `gtcx-docs/tools/audit/audit-framework` was used.
