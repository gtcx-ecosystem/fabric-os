...
**Working tree:** Dirty — untracked `docs/audit/post-roadmap-session-2026-05-30.md`
...
| ------------------------------- | ------ | --------------------------------------------------------------------------------------------- |
| `pnpm test`                     | Fail   | Docs-standard rejects `docs/roadmap/ROADMAP-2026-07-13.md`; `docs/roadmap` lacks README/index |
| `pnpm quality:governance:check` | Fail   | Same docs-standard violations as `pnpm test`                                                  |
| `pnpm typecheck`                | Fail   | `tools/replay-protection/src/middleware.mjs:130` implicit `any` parameter                     |
...
`docs/roadmap/ROADMAP-2026-07-13.md` violates lowercase filename policy, and `docs/roadmap/` has no `README.md` or `index.md`. This blocks the root quick validation gate.

**P0-002: Typecheck is red in replay-protection middleware.**  
`tools/replay-protection/src/middleware.mjs:130` defines `isExempt(path)` without a JSDoc type, triggering `TS7006` under `checkJs`.
...
`tools/replay-protection/src/middleware.mjs:130-143` treats any raw URL starting with `/_next/` as exempt. A path such as `/_next/../v1/query` satisfies the prefix check unless upstream normalization rejects it first. Normalize and reject traversal before exemption.

**P1-002: `/audit/bundles` budget and audit tenant context still trusts request headers.**  
`tools/compliance-gateway/src/audit-bundles/handler.mjs:71-74` uses `x-gtcx-tenant-id` for budget scope, and lines 126-133 write that same header into the signed ingest audit event. Tenant context should derive from resolved device identity or an authorization binding, not a caller-controlled header.

**P1-003: Auth-failure exceptions are invisible to tenant-scoped operator view.**  
`tools/compliance-gateway/src/server.mjs:231-237` signs auth failures with `payload.tenantId = 'unknown'`, while `tools/compliance-gateway/src/audit.mjs:438-446` filters exceptions by exact tenant. This drops auth-failure events from ordinary tenant exception views.

**P1-004: Auth-failure throttle is process-local and unbounded.**  
`tools/compliance-gateway/src/auth-failure-throttle.mjs:25-26` stores all source IP state in a module-level `Map` with no maximum size or eviction beyond per-IP window reset. Distributed brute-force can grow memory.

**P1-005: Source IP extraction trusts X-Forwarded-For unconditionally.**  
`tools/compliance-gateway/src/auth-failure-throttle.mjs:107-112` trusts the first XFF hop without a trusted-proxy boundary. Attackers that can inject or influence the header can evade per-IP throttling.

**P1-006: Catalog verification is self-vouching.**  
`tools/compliance-data/scripts/verify-catalog.mjs:60-64` builds the verification key from the signature file itself. Without a pinned trust anchor, replacing catalog, signature, and public key together can pass local verification.

**P1-007: Fail-closed helper is not wired into production paths.**  
`tools/scripts/fail-closed.mjs` exists and is tested, but code search found no production caller. Current fail-closed behavior remains implemented ad hoc per path.

**P1-008: Redis budget-store primitive is not wired into the hot path.**  
`tools/compliance-gateway/src/budget.mjs:22-28` explicitly documents that cross-pod budget enforcement requires a future async migration. Current rate/cost enforcement is per pod, so HPA replica count multiplies effective limits.

### P2 — Hygiene / Operational Gaps

**P2-001: Format drift across 21 files.**  
`prettier --list-different` reports docs/audit snapshots, `docs/audit/post-roadmap-session-2026-05-30.md`, docs/gitbook content, `docs/engineering/tech-stack/version-standards.md`, `docs/agile/issues/README.md`, and root `README.md`.
...
`infra/docker/docker-compose.infra.yml:151-153` defaults PagerDuty and Slack destinations to development placeholders if env vars are unset. Keep this dev-only or fail closed for staging/production compose use.

**P2-004: Current audit source is untracked.**  
`docs/audit/post-roadmap-session-2026-05-30.md` is present but untracked. Either commit it as supporting evidence or discard it before release packaging.

**P2-005: `latest.json` was stale before this audit.**  
Before this update, `docs/audit/latest.json` still pointed to the 2026-05-28 internal 10.0 assessment, despite current red gates.
