---
title: "Bank-Grade Audit Prompt (B-GAP)"
status: "current"
date: "2026-05-27"
...
**Framework Reference:** `docs/audit/audit-grade-framework.md` §2.4  
...
cat docs/audit/prompts/evidence/enterprise-grade-evidence.json 2>/dev/null | jq '.external_dependencies[] | select(.control=="E.1") | .status' || echo "Run E-GAP first"
# Check for Type II observation period documentation
cat docs/compliance/soc2-type2-observation.md 2>/dev/null | head -30 || echo "No Type II observation period doc"
# Check for 12-month evidence trail
find docs/compliance/ -name "*evidence*" -o -name "*ticket*" | wc -l
```
...
grep -ri "card\|pan\|cvv\|pci" tools/*/src/ docs/architecture/ | grep -v "discard\|token" | head -10
# Check for QSA engagement
find docs/compliance/ -name "*pci*" -o -name "*qsa*" | head -5
# Check for tokenization strategy
cat docs/architecture/payment-tokenization.md 2>/dev/null | head -30 || echo "No tokenization strategy"
```
...
cat docs/audit/red-team-scope.md 2>/dev/null | head -30 || echo "No red team scope"
# Check for previous red team report
find docs/audit/ -name "*red*team*" | head -5
# Check for assumed-breach narrative
grep -ri "assume.breach\|lateral.movement\|pivot" docs/audit/ docs/security/ | head -10
```
...
cat docs/governance/ciso-role.md 2>/dev/null | head -30 || echo "No CISO role defined"
# Check for board reporting
grep -ri "ciso\|security.*board\|monthly.*report" docs/governance/ | head -10
# Check for security org chart
cat docs/hr/security-org-chart.md 2>/dev/null | head -20 || echo "No security org chart"
```
...
cat docs/compliance/sarb-engagement.md 2>/dev/null | head -30 || echo "No SARB engagement doc"
# Check for FSCA registration
grep -ri "fsca\|category.*I\|category.*II\|license" docs/compliance/ | head -10
# Check for regulatory correspondence
find docs/compliance/ -name "*sarb*" -o -name "*fsca*" | head -5
```
...
cat docs/compliance/aml-monitoring.md 2>/dev/null | head -30 || echo "No AML monitoring doc"
# Check for SAR filing process
grep -ri "sar\|suspicious.*activity\|fic\|fincen" docs/compliance/ | head -10
# Check for sanctions screening
grep -ri "sanctions\|ofac\|un\|pep" docs/compliance/ | head -10
```
...
cat docs/compliance/sod-matrix.md 2>/dev/null | head -40 || echo "No SoD matrix"
# Check for approval workflows in code
grep -ri "approve\|approval\|dual.*control\|four.eyes" tools/*/src/ | head -10
# Check IAM for separation
grep -r "gtcx-terraform" infra/terraform/ | grep -i "role\|policy" | head -5
```
...
find docs/audit/ -name "*dr*test*" -o -name "*disaster*recovery*test*" | head -5
# Check for RTO demonstration
grep -ri "rto.*achieved\|recovery.*time.*demonstrated" docs/audit/ | head -5
# Check for external witness
find docs/audit/ -name "*witness*" -o -name "*independent*" | head -5
```
...
cat docs/financial/trust-account.md 2>/dev/null | head -30 || echo "No trust account doc"
# Check for reconciliation process
grep -ri "reconciliation\|daily\|trust.*account" docs/financial/ | head -10
# Check for CPA audit of trust account
find docs/financial/ -name "*trust*" -o -name "*reconciliation*" | head -5
```
...
cat docs/audit/pen-test-schedule.md 2>/dev/null | head -20 || echo "No pen-test schedule"
# Check for two reports in 12 months
find docs/audit/ -name "*pen*test*report*" | wc -l
# Check for remediation evidence
find docs/audit/ -name "*remediation*" | wc -l
```
...
Produce a JSON file at `docs/audit/prompts/evidence/bank-grade-evidence.json`:
...
- `docs/audit/audit-grade-framework.md` §2.4
- `docs/audit/bank-grade-rating-framework.md`
- `docs/audit/prompts/enterprise-grade-audit-prompt.md`
- `docs/compliance/soc2-gap-analysis.md`
- `docs/compliance/aml-monitoring.md`
