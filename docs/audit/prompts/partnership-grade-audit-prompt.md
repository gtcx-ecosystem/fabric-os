---
title: "Partnership-Grade Audit Prompt (P-GAP)"
status: "current"
date: "2026-05-27"
...
**Framework Reference:** `docs/audit/audit-grade-framework.md` §2.1  
...
grep -r "rateLimit\|rate_limit\|throttle" tools/compliance-gateway/src/ | head -5
grep -r "rateLimit\|rate_limit\|throttle" infra/terraform/modules/waf/ | head -5
```
...
grep -ri "72.*hour\|72h\|notification.*sla" docs/devops/ docs/security/ || true
grep -ri "incident.*response\|security.*contact" docs/ | head -5
```
...
grep -ri "ssn\|passport\|id_number\|dob\|birth" tools/compliance-gateway/src/ || true
# Check data classification
grep -ri "pii\|sensitive\|classified" docs/architecture/ || true
```
...
find docs/ -name "*partner*" -o -name "*assessment*" -o -name "*questionnaire*" | head -10
# Check for vendor security questionnaire response
find docs/ -name "*security*questionnaire*" -o -name "*vendor*assessment*" | head -5
```
...
Produce a JSON file at `docs/audit/prompts/evidence/partnership-grade-evidence.json`:
...
- `docs/audit/audit-grade-framework.md` §2.1
