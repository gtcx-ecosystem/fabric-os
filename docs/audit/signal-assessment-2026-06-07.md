...
- `compliance-gateway` on staging EKS — LLM inference path with cost-router shim (`tools/compliance-gateway/src/cost-router-shim.mjs` → baseline-os).
...
- `node tools/scripts/validate-all.mjs` — **48 gates** (coverage, static, security, build).
...
- `tools/scripts/validate-signal.mjs` — compliance scorecard gate (9.6/10).
...
- Protocol 22 work register (`docs/operations/agent-work-selection.md`); Protocol 26/27/28 cursor rules + checks.
- Audit prompts versioned under `docs/audit/prompts/`; execution-roadmap reconcile discipline.
...
- `docs/audit/latest.json` gates all green; `auto-dev-state.md` machine-readable posture.
...
- OTel collector manifest (`infra/kubernetes/base/services/otel-collector.yaml`).
...
- **Target:** Named lead + link to `gtcx-protocols/docs/operations/coordination/ai-reliability-owner-2026-06-06.md`
- **Implementation:** Update AGENTS.md + `docs/operations/coordination/README.md`; append `cross-repo-agent-log.md`
...
- **Implementation:** Export gateway LLM metrics; import baseline cost-stats JSON; add `infra/monitoring/dashboards/llm-ops.json`
...
- **Implementation:** `docs/operations/agent-pr-checklist.md` + PR template link; CI comment bot optional
...
- **Implementation:** Weekly `workflow_dispatch` or validate-all optional gate on `tools/eval-pipeline/injection-suite.mjs`

### [SIGNAL-INF-006] Infra agent failure taxonomy

- **Type:** Advance | **Priority:** P1 | **Effort:** XS
- **Dimension(s):** Safeguards, Monitoring
- **Implementation:** Copy protocols SIGNAL-006 template into `docs/operations/coordination/cross-repo-agent-log.md`
...
| 4   | Per-agent prompt version       | Semver `docs/audit/prompts/` + gateway system-prompt tags                                       |
...
- [ ] **Week 9:** Topology doc: `docs/architecture/agent-topology-2026-Q3.md`
...
node tools/scripts/validate-all.mjs          # expect 48+ pass
node tools/scripts/validate-signal.mjs       # expect ≥9.0
node --test tools/eval-pipeline/injection-suite.mjs  # if wired
...
_Assessment witness: `docs/audit/signal-assessment-2026-06-07.md` · Framework v1.0 · Assessor: platform-architect agent_
