...
1. **Supplier assessment.** All suppliers are risk-assessed before onboarding using the vendor risk program (`docs/compliance/vendor-risk-program.md`). Assessment depth is proportionate to the supplier's classification (critical, high, medium, low). No supplier is granted access before completing assessment.
