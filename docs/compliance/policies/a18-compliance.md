...
4. **Independent review.** The ISMS is independently reviewed annually by a qualified external auditor. Internal audits are conducted quarterly against the controls matrix (`docs/compliance/controls-matrix.md`). Audit findings are tracked in the risk register with remediation deadlines.

5. **Technical compliance.** Automated compliance monitoring validates: encryption standards, access controls, logging configuration, and vulnerability posture. The SOC 2 evidence pipeline (`docs/compliance/soc2-evidence-pipeline.md`) continuously collects evidence for audit readiness. Deviations trigger automated alerts.
