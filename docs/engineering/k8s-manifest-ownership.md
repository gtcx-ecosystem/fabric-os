...
```
gtcx-infrastructure/infra/kubernetes/     ← Platform team
...
| **Prometheus alerts** | `infra/monitoring/alerts/protocol-alerts.yml`               | `deploy/monitoring/prometheus-alerts.yaml`   | Duplicate alert definitions for same service         |
| **SLO rules**         | `infra/monitoring/rules/slo-recording-rules.yml`            | `deploy/monitoring/slo-recording-rules.yaml` | Duplicate recording rules                            |
| **Deployment**        | `infra/kubernetes/base/services/api.yaml`                   | `deploy/k8s/deployment.yaml`                 | Two deployment manifests for protocols               |
| **Service**           | `infra/kubernetes/base/services/protocols.yaml` (ClusterIP) | `deploy/k8s/service.yaml`                    | Two service definitions                              |
...
| Namespace                              | **Infrastructure** | `gtcx-infrastructure/infra/kubernetes/base/`                        | Platform owns namespace lifecycle                     |
| Network policy                         | **Infrastructure** | `gtcx-infrastructure/infra/kubernetes/overlays/`                    | Platform owns security boundaries                     |
| Service mesh (Linkerd)                 | **Infrastructure** | `gtcx-infrastructure/infra/kubernetes/overlays/production/linkerd/` | Platform decision — one mesh for all services         |
| SLO recording rules                    | **Infrastructure** | `gtcx-infrastructure/infra/monitoring/rules/`                       | Platform owns SLO definitions                         |
| Ingress / tunnel routing               | **Infrastructure** | `gtcx-infrastructure/infra/kubernetes/base/services/cloudflared/`   | Platform owns external routing                        |
| Kyverno admission policies             | **Infrastructure** | `gtcx-infrastructure/infra/kubernetes/base/policies/`               | Platform owns admission control                       |
| Resource quotas                        | **Infrastructure** | `gtcx-infrastructure/infra/kubernetes/overlays/`                    | Platform owns capacity limits                         |
...
1. **Delete `infra/kubernetes/base/services/api.yaml`** — protocols repo owns the deployment. Infrastructure was maintaining a duplicate.

2. **Delete `infra/monitoring/alerts/protocol-alerts.yml`** — protocols repo owns its alerts. Infrastructure will consume them via Prometheus federation or shared rules path.

3. **Keep and own:**
   - `infra/kubernetes/base/services/cloudflared/` — tunnel routing
   - `infra/kubernetes/base/services/nats.yaml` — NATS is platform infrastructure
   - `infra/kubernetes/base/services/otel-collector.yaml` — observability infra
   - `infra/kubernetes/base/policies/` — all Kyverno admission policies
   - `infra/kubernetes/overlays/production/linkerd/` — service mesh
   - `infra/kubernetes/overlays/production/network-policies.yaml` — security boundaries
   - `infra/monitoring/rules/slo-recording-rules.yml` — platform SLO definitions
