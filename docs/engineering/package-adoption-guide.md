...
```typescript
// src/crypto.ts — duplicated in 12 repos
...
```typescript
// src/crypto.ts — single source of truth
...
- name: Check for crypto re-implementation
  run: |
    ...
    if grep -r "createHash('sha256')" src/ || grep -r "ed25519" src/ --include="*.ts" | grep -v "@gtcx/protocols-crypto"; then
...
- [ ] `pnpm build` passes
- [ ] `pnpm lint` passes
- [ ] No `createHash`, `ed25519`, or `jsonschema` found in `src/` outside `@gtcx` imports

### References

- [Package Adoption Guide](https://github.com/gtcx-ecosystem/gtcx-infrastructure/blob/main/docs/engineering/package-adoption-guide.md)
