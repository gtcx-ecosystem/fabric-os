...
- `infra/kubernetes/overlays/staging/ingress.yaml` — added `/v1/tradepass` route
- `infra/kubernetes/base/kustomization.yaml` — added did-resolver resources
- `infra/kubernetes/base/services/did-resolver/*` — new static DID resolver
- `infra/terraform/modules/waf/main.tf` — added `AllowAuditAndTradePassEndpoints` rule
- `infra/terraform/environments/staging/main.tf` — enabled `allow_audit_paths`
- `infra/terraform/modules/alb/main.tf` — added `RemoveListenerCertificates` permission
