...
2. `build-push.sh` intelligence-sdk now uses `gtcx-intelligence/intelligence/sdk/Dockerfile` (not legacy `infra/docker/Dockerfile.intelligence`).

## Operator commands (Class R — run in-session)

```bash
# 1) Build + push (gtcx-infrastructure, Docker + AWS CLI required)
bash infra/scripts/build-push.sh intelligence-sdk --version=b0488d2d955471f439b824309dd5e15264d4ce53
