...
> **Workspace type:** Monorepo (pnpm + Turborepo, 15 workspace packages under `tools/`).
...
Keep the repository root predictable for humans and agents. Only intentional files live at root; everything else belongs under `docs/`, `infra/`, `scripts/`, `tools/`, `workspace/`, or `agents/`.
...
| Dir          | Purpose                                                     |
| ------------ | ----------------------------------------------------------- |
| `docs/`      | All documentation, audits, runbooks, coordination           |
| `infra/`     | Terraform, Kubernetes, Docker, security                     |
| `scripts/`   | Repo automation, agent selection, production operators      |
| `tools/`     | Workspace packages (compliance-gateway, audit-flush, etc.)  |
...
Checker: `scripts/ops/check-workspace-root-cleanliness.py` (adopted from `compliance-os` / `gtcx-core`).

Wired in `tools/scripts/validate-all.mjs` and `.github/workflows/ci.yml`.
...
Registry: `gtcx-docs/tools/audit/audit-framework/commands/execute-repo-hygiene.md`
