...
Each Prometheus alert in `infra/monitoring/alerts/` has its `runbook_url`
...
`tools/scripts/alerts-add-runbook-url.mjs --check`) enforces the
...
| Source   | `infra/monitoring/alerts/<file>.yml`                            |
...
| -------- | --------------------------------------------- |
| Severity | critical                                      |
| Source   | `infra/monitoring/alerts/protocol-alerts.yml` |
...
**Triage:** Check NATS broker latency, DB connection pool saturation, recent traffic spike. Compare against k6 soak baseline in `tools/load-tests/`.
...
Rate-limit rejections > 10/min. Investigate source IP via access logs; if legitimate traffic, scale HPA upper bound or tune per-principal budget in `tools/compliance-gateway/src/budget.mjs`.
...
**For any fast-burn page:** consult the SLO dashboard (`infra/monitoring/dashboards/gtcx-slo-dashboard.json`) to identify the contributing endpoint, then follow the protocol or intelligence runbook for that service.
...
1. Add the alert to `infra/monitoring/alerts/<area>.yml`.
2. Run `node tools/scripts/alerts-add-runbook-url.mjs` to add the `runbook_url` annotation.
