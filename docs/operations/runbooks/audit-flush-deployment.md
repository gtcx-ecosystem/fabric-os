...
The contract lives in `infra/kubernetes/base/services/audit-flush.yaml`; pin the digest in the kustomize image overlay per environment.

```bash
# Build + push (replace REGISTRY)
docker build -t $REGISTRY/audit-flush:$(git rev-parse --short HEAD) tools/audit-flush
...
yq -i \
  ...
  infra/kubernetes/overlays/testnet/kustomization.yaml
```
...
| Verification fails on a downloaded NDJSON | Tampering OR mid-flight write                       | Pull again from a different time window; if reproducible, escalate per `docs/security/break-glass-procedure.md`. |
