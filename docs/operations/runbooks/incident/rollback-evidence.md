...
The evidence bundle is generated locally and written under `04-deploy/security/reports/rollback-evidence/`. That path is already ignored by git and should stay uncommitted unless a separate evidence retention process requires export.

## Capture Command

```bash
./04-deploy/03-platform/scripts/capture-rollback-evidence.sh testnet-pilot \
...
```bash
./04-deploy/03-platform/scripts/capture-rollback-evidence.sh production \
