...
```bash
kubectl apply -k infra/kubernetes/overlays/staging/monitoring/
...
# Import infra/monitoring/dashboards/llm-ops.json via UI or API
```
...
```bash
baseline cost-stats --json > docs/audit/evidence/baseline-cost-stats-latest.json
```

## Evidence

Write `docs/audit/evidence/signal-sprint2-monitoring-latest.json` after verify.
