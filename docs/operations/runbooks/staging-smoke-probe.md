...
```bash
kubectl apply -f infra/kubernetes/overlays/staging/smoke-probe-cronjob.yaml
```
...
FROM node:22-alpine
WORKDIR /app
COPY tools/control-plane/capture-runtime-smoke-evidence.mjs ./
...
Commit the collected evidence to `infra/security/reports/runtime-smoke-evidence/staging/` and link from `docs/audit/latest.json`.
