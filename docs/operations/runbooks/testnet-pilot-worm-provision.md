...
```bash
mkdir -p infra/security/reports/testnet-pilot-worm-evidence/$(date -u +%Y%m%d-%H%M%S)
```
...
Commit these to `infra/security/reports/testnet-pilot-worm-evidence/<timestamp>/` and link from `docs/audit/latest.json`.
