---
title: "Security"
status: "current"
date: "2026-05-27"
owner: "gtcx-infrastructure"
role: "protocol-architect"
agent_id: "agent://gtcx-infrastructure/2026-05-27/session-backfill"
trust_score: 95
autonomy_level: "sovereign"
tier: "critical"
tags: ["documentation", "security"]
review_cycle: "on-change"
---

---
title: 'Security'
status: 'current'
date: '2026-05-24'
owner: 'frontier-infra-engineer'
role: 'crypto-security-engineer'
tier: 'critical'
tags: ['security']
review_cycle: 'on-change'
---

# Security

This directory contains security documentation for the GTCX infrastructure.

## Contents

- audit-integrity-verification.md
- aws-security-monitoring.md
- break-glass-procedure.md
- bug-bounty-activation-checklist.md
- bug-bounty-policy.md
- cosign-ci-integration.md
- cve-acceptance-log.md
- data-flow.md
- defense-readiness.md
- fips-assessment.md
- forensic-readiness.md
- key-ceremony-runbook.md
- nist-800-53-mapping.md
- rasp-integration-guide.md
- red-team-containment-narrative-template.md
- red-team-playbook.md
- secrets-management.md
- security-architecture.md
- security-framework.md
- security-policy.md
- security-training-program.md
- signed-commits-policy.md
- soc-requirements.md
- stig-compliance.md
- threat-model-template.md
- threat-model.md
- tokenization-architecture.md
- zero-trust-assessment.md

---

_Generated: 2026-05-17_
