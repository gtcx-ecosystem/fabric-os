...
jobs:
  verify-governance:
    ...
    steps:
      - name: Verify M-of-N approvals
        run: node scripts/verify-governance-contract.mjs \
  ...
  execute-ceremony:
    ...
    steps:
      ...
      - name: Export SPKI
        run: node scripts/export-spki.mjs --authority ${{ inputs.authority }} --ceremony-id ${{ inputs.ceremony_id }}

      - name: Generate ceremony evidence
        run: node scripts/generate-ceremony-evidence.mjs --ceremony-id ${{ inputs.ceremony_id }}
```
...
4. Implement `scripts/verify-governance-contract.mjs` for M-of-N agent approval.
5. Implement `scripts/witness/` suite for independent verification.
