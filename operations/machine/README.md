# workspace/product-management — gtcx-infrastructure

**Protocol 22** + **gtcx-agile** bridge. Machine backlog — not the aggregation hub (unless this repo is gtcx-agile).

| Artifact            | Path                               |
| ------------------- | ---------------------------------- |
| Manifest            | [`manifest.json`](./manifest.json) |
| Backlog (generated) | [`backlog.json`](./backlog.json)   |

```bash
pnpm generate:roadmap   # machine/daas-*.json → audit/product-management/execution-roadmap.md
pnpm machine:sync            # refresh backlog from agent:next-work + roadmap
pnpm machine:status
```

**Narrative:** [`01-docs/05-audit/agile/roadmap.md`](../../01-docs/05-audit/agile/roadmap.md)  
**Hub workplan:** [gtcx-agile cross-repo sprint](https://github.com/gtcx-ecosystem/gtcx-agile/blob/main/01-docs/06-coordination/cross-repo-sprint-workplan-2026-06.md)
