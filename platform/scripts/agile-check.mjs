...
function main() {
  ...
  for (const absent of spec.forbiddenUnderDocsAbsent ?? []) {
    const abs = join(REPO, absent.path);
    gates.push(
      gate(
        `absent:${absent.path.replace(/\//g, '-')}`,
        !existsSync(abs),
        existsSync(abs) ? `delete ${absent.path} — use ${absent.canonical}` : 'absent',
      ),
    );
  }
