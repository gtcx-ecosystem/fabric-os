...
const WITNESS = join(REPO, 'audit/evidence/docs-six-pack-latest.json');

const IA_MAP_URL =
  'github.com/gtcx-ecosystem/canon-os/blob/main/docs/governance/docs-ia/IA-PILLAR-MAP.md';
const SIX_PACK_SPEC_URL = 'github.com/gtcx-ecosystem/canon-os/blob/main/pm/spec/docs-six-pack.json';
...
function isHubProfile(repoRoot) {
  const sorPath = join(repoRoot, 'docs/sor.json');
  if (existsSync(sorPath)) {
    try {
      return JSON.parse(readFileSync(sorPath, 'utf8')).repoKind === 'constitution-standards';
    } catch {
      return false;
    }
  }
...
function main() {
  ...
  if (indexText) {
    ...
    gates.push(
      gate(
        'index:thin-instance',
        /thin instance|P48 thin|Fleet normative/i.test(indexText),
  ...
  if (readmeText) {
    ...
    gates.push(
      gate(
        'readme:p48-six-pack',
        /P48|six-pack|thin six-pack/i.test(readmeText),
        'mentions P48 six-pack',
      ),
    );
  }
  ...
  if (convText && !isHub) {
    gates.push(
      gate(
        ...
        convText.includes(HUB_CONVENTIONS_URL) || convText.includes('canon-os') && /conventions/i.test(convText),
        HUB_CONVENTIONS_URL,
      ),
    );
  }
  if (convText && hubFolderSpec) {
    gates.push(gate('conventions:hub-sor', convText.includes('six-pack') || convText.includes('INDEX.md'), 'hub conventions SoR'));
  }
...
function emit(gates, repoName, hubFolderSpec) {
  ...
  const witness = {
    schema: 'gtcx://canon-os/docs-six-pack-check/v1',
    ...
    fleetSoR: {
      iaMap: IA_MAP_URL,
      sixPackSpec: SIX_PACK_SPEC_URL,
      docsRootSpec: DOCS_ROOT_SPEC_URL,
    },
  };

  console.log('=== docs:six-pack:check ===\n');
