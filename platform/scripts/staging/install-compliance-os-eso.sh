...
OVERLAY="${REPO_ROOT}/deploy/kubernetes/overlays/staging/compliance-os"
...
kubectl get sa compliance-os-sa -n compliance-os-staging -o jsonpath='{.metadata.annotations.eks\.amazonaws\.com/role-arn}{"\n"}' \
  ...
  echo "Run: terraform -chdir=deploy/terraform/environments/staging apply -target=module.secrets"
}
