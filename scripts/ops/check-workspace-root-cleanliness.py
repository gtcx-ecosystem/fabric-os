#!/usr/bin/env python3
"""Validate that the repo root matches docs/operations/repo/root-allowlist.json.
...
ALLOWLIST_PATH = ROOT / "docs/operations/repo/root-allowlist.json"
SCHEMA_PATH = ROOT / "docs/operations/repo/root-allowlist.schema.json"
...
# A path like `scripts/coverage/check.mjs` matches BUILD_ARTIFACT_RE because
...
def find_issues(allowlist: dict, root: pathlib.Path | None = None) -> list[str]:
    ...
    for entry in sorted(base.iterdir(), key=lambda path: path.name.lower()):
        ...
        if name not in allowed:
            hint = " See docs/operations/repo/repo-hygiene-protocol.md for canonical homes."
            issues.append(
...
def _is_build_artifact(path: str) -> bool:
    ...
    RH-P2-01: scripts/coverage/check-summary.mjs is a coverage CHECKER, not
...
def compute_deterministic_axes() -> dict:
    ...
    after_ignore = [p for p in candidates if p not in ignored]

    # Third pass (RH-P2-03): an empty dir matters only if it would persist on a
    # fresh clone, i.e. has at least one tracked file underneath. Local-only
    # empty dirs (untracked filesystem state) are silent — they vanish on
    # clone and never affect a teammate's working tree.
    tracked_set = set(tracked)
    empty_dirs = sorted(
        p for p in after_ignore if any(t.startswith(f"{p}/") for t in tracked_set)
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("")
    print("Policy: docs/operations/repo/repo-hygiene-protocol.md")
    print(
