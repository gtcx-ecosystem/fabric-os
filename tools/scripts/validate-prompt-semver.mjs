...
const out = execSync('node -e "import(\'./tools/compliance-gateway/src/system-prompt.mjs\').then(m=>console.log(JSON.stringify(m.getPromptMetadata())))"', {
  cwd: ROOT,
  encoding: 'utf8',
});
const meta = JSON.parse(out.trim());
const version = meta.promptVersion;
const dir = path.join(ROOT, `docs/audit/prompts/compliance-gateway@${version}`);
